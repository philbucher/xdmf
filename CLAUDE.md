...
`Error` / `Result<T>` (`src/error.rs`) is the crate's error type: a flat `thiserror` enum covering construction, mesh, time-step, attribute, and storage-specific failures, plus `Error::Io`/`Error::Hdf5` (the latter `cfg`-gated on the `hdf5` feature) that wrap the underlying `std::io::Error`/`hdf5::Error` with an operation description and (for `Io`) a path — attached via the `error::io_ctx(operation, path)` helper at every fallible filesystem call, never via a bare `?`. There is deliberately no blanket `From<std::io::Error> for Error`, since that would lose exactly that context; there is a `From<Error> for std::io::Error` for callers that plumb `io::Error` throughout their own codebase. A handful of variants (`DataWriteNotInitialized`/`DataWriteAlreadyInitialized`/`Hdf5PathResolution`/`MissingFileNameComponent`) guard internal state-machine invariants inside the `*_writer.rs` backends that are not reachable through the public `TimeSeriesWriter` API.
...
- A test that asserts an operation fails must check *which* error occurred, not just `is_err()`/`unwrap_err()` with nothing else. Errors are the typed `xdmf::Error` (`src/error.rs`), which is not `PartialEq` (it wraps `std::io::Error`/`hdf5::Error`), so assert on the **variant and its fields** with `matches!`, e.g. `assert!(matches!(writer.write_data("0.0", data, []).unwrap_err(), Error::DataSizeMismatch { expected: 10, actual: 9, .. }))` — see `time_series_writer.rs`. Message-text assertions belong only in `src/error.rs`'s `mod error_messages`, which keeps one `Display` test per message family (not one per call site) so wording stays guarded without recoupling every test to exact strings.

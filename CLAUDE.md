...
- A test that asserts an operation fails must check *which* error occurred, not just `is_err()`/`unwrap_err()` with nothing else. Since errors here are `std::io::Error` (no `PartialEq`), the established pattern is `assert_eq!(res.unwrap_err().to_string(), "expected message")` — see `time_series_writer.rs`.

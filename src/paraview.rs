...
//!
//! [`validate`] is the single entry point, called by [`TimeSeriesWriter`] and [`TimeStep`] and
//! never by a [`DataWriter`] backend. It only ever *rejects*, never rewrites, so a path that does
//! not care about `ParaView` is this one call away.
//!

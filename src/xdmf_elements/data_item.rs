...
impl Format {
    ...
    /// Paraview's legacy Xdmf2 reader silently misreads 64-bit integers (signed or unsigned) in
    /// binary format, thus narrowing to 32-bit for both `Values::U64` and `Values::I64`.
    pub(crate) fn int64_precision(self) -> u8 {
...
mod tests {
    ...
    #[test]
    fn format_int64_precision() {
        assert_eq!(Format::XML.int64_precision(), 8);
        assert_eq!(Format::HDF.int64_precision(), 8);
        assert_eq!(Format::Binary.int64_precision(), 4);
    }

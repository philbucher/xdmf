...
impl Xdmf {
    ...
    ///
    /// The `writer`'s own [`std::io::ErrorKind`] is reported unchanged; a serialization failure,
    /// which would be a bug in these element types, keeps [`std::io::ErrorKind::Other`].
